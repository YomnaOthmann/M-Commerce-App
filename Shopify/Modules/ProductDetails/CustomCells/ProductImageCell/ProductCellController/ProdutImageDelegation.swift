...
class ProdutImageDelegation: NSObject, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UICollectionViewDelegate {
    ...
    init(collectionView: UICollectionView, with imageIndicator: UIPageControl, itemDetails: Product) {
        ...
        // Check if itemDetails.images is not nil before accessing its count
        self.imageIndicator.numberOfPages = itemDetails.images.count 
    }

    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return itemDetails.images.count 
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        // Directly use the value, as it's not optional
        let imageExistance = itemDetails.images[indexPath.row].src
    ...
    func collectionView(_ collectionView: UICollectionView, didEndDisplaying cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        if imageIndicator.currentPage == indexPath.row {
            imageIndicator.currentPage = collectionView.indexPath(for: collectionView.visibleCells.first!)?.row ?? 0
        }
    }
}
